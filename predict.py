...
for symbol in symbols:
    ...
    with st.spinner(f"Fetching data for {symbol}..."):
        try:
            ...
            fig.update_layout(title=f"{symbol} Stock Price", xaxis_title="Date", yaxis_title="Price (USD)", template=template)
            ...
            if symbol in portfolio:
                ...
                purchase_price = st.sidebar.number_input(f"Purchase price of {symbol} (USD)", min_value=0.0, value=0.0)
            ...
            # Alert System
            st.subheader("🚨 Alerts")
            alert_price = st.number_input(f"Set price alert for {symbol}", min_value=0.0, value=0.0, step=1.0)
            volatility = data['Close'].pct_change().rolling(5).std().iloc[-1] * 100
            if current_price >= alert_price > 0:
                st.warning(f"📢 Alert: {symbol} has hit your alert price of ${alert_price:.2f}!")
            if volatility > 5:
                st.error(f"⚠️ Volatility spike detected: {volatility:.2f}%")

            # News Widget (basic)
            st.subheader("📰 Daily News (Mock)")
            st.info(f"Placeholder news headline for {symbol}: 'Earnings beat expectations this quarter.'")
